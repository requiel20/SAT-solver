...
void CLAUSEallocate(V literals, bool learnt, C *output) {
...
bool CLAUSEnew(V literals, bool learnt, C *output) {
    ...
    if (VECTORtotal(literals) == 1) {
        ...
        CLAUSEallocate(literals, learnt, output);
...
bool findNewWatcher(C clause) {
    Var p = VECTORget(clause->literals, 1);
